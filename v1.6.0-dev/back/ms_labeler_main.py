...
def ms_labeler(camera_list_path:str, weight_name:str, zero_shot_flag):
    ...
    camera_info = load_crypography_json(os.path.join(os.getcwd(), "cache", "camera_info.json"))

    ALARM_TYPE_DIC = {2 : "침입 ", 1 : "배회", 6 : "쓰러짐", 4 : "방화", 7 : "싸움", 5 : "무단투기"}
    camera_num_to_camera_name = {}
    for camera_name in camera_info:
        camera_num_to_camera_name[camera_info[camera_name]["Num"]] = camera_name
    
    print(camera_num_to_camera_name)
    ...
    auth = HTTPBasicAuth(login_info["NVR"]["ID"], login_info["NVR"]["PW"]) # NVR에 대한 ID / PW
    event_info_post = f'http://{login_info["NVR"]["IP"]}/api/events?types=70&since={video_time_start}&total=true&limit=1000&sort=1'
    r = requests.get(event_info_post, auth=auth, timeout= 1)
    event_info_ori = r.json()
    labelint_video_list = []
    for event in event_info_ori["events"]:
        labelint_video_list.append({"camera_name": camera_num_to_camera_name[event["devices"][0]+1], 
    ...
    for video_info in labelint_video_list :
        try:
            ...
            nvr_username = login_info["NVR"]["ID"]
            nvr_password = login_info["NVR"]["PW"]
            ...
            with torch.no_grad():
                ...
                data_save_dir = os.path.join(os.getcwd(), ".." ,"backup", "dataset", NVR_ID, camera_name, save_date)
