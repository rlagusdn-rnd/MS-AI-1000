...
@app.post("/detect_siglip")
async def detect_siglip_endpoint(data: SiglipDetectionData):
    """SigLip 이미지-텍스트 유사도 측정 API 엔드포인트"""
    global siglip_model, siglip_processor
    
    try:
        t0 = time.time()
        logger.info(f"SigLip 검출 요청 - 카메라: {data.camera_name}, Person ID: {data.person_id}, Type: {data.detect_type}")
        
        if siglip_model is None or siglip_processor is None:
            logger.error("SigLip 모델이 로드되지 않음")
            return {"status": False, "probs": [], "error": "SigLip model not loaded"}
        ...
        # SigLip 처리
        inputs = siglip_processor(text=data.texts, images=pil_img, padding="max_length", return_tensors="pt")
        ...
        logger.info(f"SigLip 검출 완료 - 소요시간: {time.time() - t0:.2f}s,  카메라: {data.camera_name}, Person ID: {data.person_id}, Type: {data.detect_type} 확률: {np.round(probs_list, 2)}")
        
        # 메모리 정리
        del img, img_rgb, pil_img, inputs, outputs, logits_per_image, probs
        cleanup_memory()
        ...
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        tb = traceback.format_exc()
        logger.error(f"SigLip 검출 에러 발생 at {current_time}: {e}\n{tb}")
        cleanup_memory()
        return {"status": False, "probs": [], "error": f"Internal server error: {str(e)}"}
