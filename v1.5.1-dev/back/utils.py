...
def send_SERVER_ai_info(host, port, camera_info_dict, video_save_buffer, nvr_ip, video_save_flag):
    ...
    for camera_num, camera_info in camera_info_dict.items():
        ...
        if video_save_flag:
            video_save_buffer_update(camera_info["name"], camera_info, video_save_buffer)
...
def send_alarm_NVR_mp(nvr_ip, nvr_id, nvr_pw, camera_info_dict, camera_alarms, ROI_color_dict, object_color_dict):
    """멀티프로세스 환경용 NVR 알람 전송 함수"""
    try:
        auth = HTTPBasicAuth(nvr_id, nvr_pw)  # NVR에 대한 ID / PW
        ...
        for camera_num, camera_info in camera_info_dict.items():
            ...
            # bbox 정보가 있는 경우 SVG에 추가
            if "person_bbox" in camera_info and "non_person_bbox" in camera_info and "img" in camera_info:
                svg_data = get_svg_bbox(svg_data, 
                                        camera_info["person_bbox"], 
                                        camera_info["non_person_bbox"], 
                                        ori_imsz=(camera_info["img"].shape[1], camera_info["img"].shape[0]), 
            ...
            # 알람이 있는 경우 이벤트 전송
            if camera_name in camera_alarms and len(camera_alarms[camera_name]):
                data = {}
                headers = {"Content-Type": "application/json"}

                for alarm_info in camera_alarms[camera_name]:
                    detect_type = alarm_info[0]
                    detect_num = get_detect_num(detect_type)
                    data = {"type": 70, "devices": [int(camera_num) - 1], "micro_ai": {"type": detect_num, "object": 1}}
                    r = requests.put(event_post, headers=headers, json=data, auth=auth)
                    ...
                    current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    print(f"[{current_time}] NVR 알람 전송: 카메라 {camera_name} - {detect_type}", file=sys.stderr)

    except Exception as e:
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        tb = traceback.format_exc()
        print(f"NVR 알람 전송 에러 발생 : {current_time}: {e}\n{tb}", file=sys.stderr)

def send_SERVER_ai_info_mp(host, port, camera_info_dict, camera_alarms, video_save_buffer, nvr_ip, video_save_flag):
    """멀티프로세스 환경용 서버 AI 정보 전송 함수"""
    try:
        url = f"http://{host}:{port}/upload-ai-data"
        data = {}
        ...
        for camera_num, camera_info in camera_info_dict.items():
            ...
            # bbox 정보가 있는 경우 처리
            if "person_bbox" in camera_info:
                person_bbox = camera_info["person_bbox"]
                
                # person_bbox에 status 정보 추가 (멀티프로세스에서는 status를 0으로 설정)
                add_status_person_bbox = []
                for person_box in person_bbox:
                    if len(person_box) == 8:
                        x1, y1, x2, y2, id_, conf, cls, ind = person_box
                        # 멀티프로세스 환경에서는 HAR 정보가 분리되어 있으므로 status를 0으로 설정
                        status = 0
                        add_status_person_bbox.append([x1, y1, x2, y2, int(id_), conf, cls, ind, int(status)])
                    else:
                        add_status_person_bbox.append(person_box)
                
                person_bbox = add_status_person_bbox
            
            if "non_person_bbox" in camera_info:
                non_person_bbox = camera_info["non_person_bbox"]
            
            # 데이터 타입 변환
            person_bbox = person_bbox.tolist() if isinstance(person_bbox, np.ndarray) else person_bbox
            non_person_bbox = non_person_bbox.tolist() if isinstance(non_person_bbox, np.ndarray) else non_person_bbox
            ROI_ori = camera_info["ROI_ori"]

            # 알람 정보 추가
            alarms = camera_alarms.get(camera_name, [])
            ...
            # 비디오 저장 처리
            if video_save_flag and "img" in camera_info:
                # video_save_buffer 업데이트
                if camera_name not in video_save_buffer:
                ...
                # 알람이 있는 경우 비디오 저장 스케줄링
                if alarms:
                    for alarm in alarms:
                        timer = threading.Timer(5, start_delayed_video_save_mp, 
                                              args=(video_save_buffer, camera_name, alarm, nvr_ip))
                        timer.start()
        ...
        if response.status_code == 200:
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            alarm_count = sum(len(alarms) for alarms in camera_alarms.values())
            if alarm_count > 0:
                print(f"[{current_time}] 서버 AI 정보 전송 완료: {alarm_count}개 알람", file=sys.stderr)
        else:
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            print(f"[{current_time}] 서버 AI 정보 전송 실패: HTTP {response.status_code}", file=sys.stderr)

    except Exception as e:
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        tb = traceback.format_exc()
        print(f"서버 AI 정보 전송 에러 발생 : {current_time}: {e}\n{tb}", file=sys.stderr)

def start_delayed_video_save_mp(video_save_buffer, camera_name, alarm, nvr_ip):
    """멀티프로세스 환경용 지연 비디오 저장 시작 함수"""
    thread = threading.Thread(target=video_save_mp, args=(video_save_buffer, camera_name, alarm, nvr_ip))
    thread.start()

def video_save_mp(video_save_buffer, camera_name, alarm, nvr_ip):
    """멀티프로세스 환경용 비디오 저장 함수"""
    try:
        if camera_name not in video_save_buffer or "img_ori_buffer" not in video_save_buffer[camera_name]:
            print(f"비디오 버퍼가 없습니다: {camera_name}", file=sys.stderr)
            return
            
        img_buffer_ori = video_save_buffer[camera_name]["img_ori_buffer"]
        
        if len(img_buffer_ori) == 0:
            print(f"비디오 버퍼가 비어있습니다: {camera_name}", file=sys.stderr)
            return
        ...
        # 출력 경로 설정
        output_video_ori_file_path = os.path.join(os.getcwd(), "..", "backup", nvr_ip, 
                                                  f"{camera_name}", new_date, "videos_ori")
        os.makedirs(output_video_ori_file_path, exist_ok=True)

        output_video_ori_name = os.path.join(output_video_ori_file_path, 
                                           f"{new_time}_{Eng2kor(alarm[0])}.avi")

        # 비디오 작성기 초기화
        if len(img_buffer_ori) > 0 and img_buffer_ori[0] is not None:
            height, width = img_buffer_ori[0].shape[:2]
            writer_ori = cv2.VideoWriter(output_video_ori_name, fourcc, fps, (width, height))

            if not writer_ori.isOpened():
                print(f"비디오 작성기 초기화 실패: {output_video_ori_name}", file=sys.stderr)
                return

            # 프레임 작성
            frame_count = 0
            for img in img_buffer_ori:
            ...
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            print(f"[{current_time}] 비디오 저장 완료: {output_video_ori_name} ({frame_count} 프레임)", file=sys.stderr)

    except Exception as e:
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        tb = traceback.format_exc()
        print(f"비디오 저장 에러 발생 {current_time}: {e}\n{tb}", file=sys.stderr)
        try:
            if 'writer_ori' in locals():
                writer_ori.release()
        except:
            pass
...
def camera_info_refresh_mp(camera_info_dict, camera_alarms, start_time):
    ...
    try:
        ...
        if current_time - start_time > 3600:
            ...
            print(f"[{current_time_str}] 알람 정보 및 메모리 초기화 완료", file=sys.stderr)
        ...
        current_time_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        tb = traceback.format_exc()
        print(f"카메라 정보 갱신 에러 발생 : {current_time_str}: {e}\n{tb}", file=sys.stderr)
        return camera_info_dict, start_time