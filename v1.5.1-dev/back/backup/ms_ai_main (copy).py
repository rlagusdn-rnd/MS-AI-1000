    ...
    try:
        ...
        if id_ in camera_info["fight"] and camera_info["fight"][id_][1] == -1:
            if result:  # VLM 응답이 True인 경우
                ...
                camera_info["fight"][id_][0] = time.time() + 30
                logger.info(f"VLM 싸움 검출 실패, 30초 후 재시도: camera={camera_name}, id={id_}")
        ...
        # falldown 검출 결과 처리
            ...
            if result:  # VLM 응답이 True인 경우
                ...
                logger.info(f"VLM 쓰러짐 검출 실패, 30초 후 재시도: camera={camera_name}, id={id_}")
