...
# 로깅 설정
logger = setup_logging(logger_name="MS_AI_MAIN")
...
class Video_Buffer:
    def __init__(self, pipe="video1", appsink_name="video_sink", resolution = (640, 480)):
        ...
        try:
            ...
            self.video_decode = f'!   ! videoscale ! video/x-raw,width={resolution[0]},height={resolution[1]} ! videoconvert ! video/x-raw,format=(string)BGR ! appsink name={appsink_name} emit-signals=true sync=false max-buffers=3 drop=true'
...
#     """싸움 검출 시작 기준 : 두 사람 이상의 bbox가 iou 0.01 이상 겸쳐질 경우
#        싸움 검출 기준 : 이미지와 텍스트의 유사도 점수가 일정 값 이상(90점)일 경우 쓰러짐 검출
#        싸움 알림 발생 기준 : 쓰러짐 검출이 10회 중 5회 이상 발생하고 5초 이상 지속될 경우 VLM에 QnA 요청 후 답변이 yes일 경우 알림 발생
#     """
#     try:
...
def detect_fight(camera_info, detect_area, person_bbox_list, siglip_model, siglip_processor):
    """싸움 검출 시작 기준 : 사람의 bbox 하단 점이 검출 영역에 들어올 경우
       싸움 검출 기준 : 이미지와 텍스트의 유사도 점수가 일정 값 이상(30점)일 경우 싸움 검출
        ...
        if len(person_bbox_list):
            for x1, y1, x2, y2, id_, conf, label, _ in person_bbox_list:
                ...
                if check_box_in_area(detect_point, detect_area):
                    ...
                    if probs[0][0] > 0.3:
                        camera_info["person_info"].update_status(id_, detect_type = "fight", status = 2) #싸움 검출 상태 할당
...
def detect_action(camera_info, siglip_model, siglip_processor):
    ...
    for detect_info in camera_info["TF_ROI"]:
        ...
        elif detect_type == "Falldown" and siglip_model is not None:
        ...
        elif detect_type == "Fight" and siglip_model is not None:
...
def ms_ai(camera_info_dict_ori, NVR_info, weight_name, video_save_flag):
    ...
    if active_HAR:
        ...
        siglip_model = SiglipModel.from_pretrained(str(os.getcwd()) + f"/../weights/SigLip_512",
        ...
        if len(img_list):
            ...
            for index, bbox in enumerate(bn_person_boxes):
                ...
                try: 
                    ...
                    camera_info_dict[camera_num_list[index]]["person_info"].update_id(img = img_list[index],
            ...
            for camera_num, camera_info in camera_info_dict.items():
                """지능형 알고리즘 실행"""
                camera_info = detect_action(camera_info = camera_info,
                                            siglip_model = siglip_model,
                                            siglip_processor = siglip_processor,
                                            )
                # print(camera_name)
