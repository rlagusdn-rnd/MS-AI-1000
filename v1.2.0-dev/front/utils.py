...
class Connect_Camera(QThread):
    ...
    def __init__(self, pipe, host, port, camera_name, camera_num, viewer, plot_detect = False, plot_bbox = True, plot_label = True,  roi_thickness = 1, save_img_flag = False) -> None:
    ...
    def run(self) -> None:
            ...
            while self.__thread_active:
                # print(f"camera : {self.pipe}", cap.frame_available())
                if cap.frame_available() and not self.__thread_pause:
                    ...
                    if self.plot_detect and receive_data[self.camera_name]:
                        ...
                        frame_alarm = plot_detect_info(img = self.frame_ori.copy(), detect_info = receive_data[self.camera_name], roi_thickness = self.roi_thickness, plot_bbox = self.plot_bbox, plot_label = self.plot_label)

                        self.img_buffer.append(frame_alarm)
                        self.img_buffer_ori.append(self.frame_ori.copy())


                    if len(self.img_buffer) > 300:
                        self.img_buffer.pop(0)
                        self.img_buffer_ori.pop(0)
                ...
                # else:
                #     print(f"disconnect camera : {self.pipe}")
                    # self.disconnect_cnt += 1

                    # if self.disconnect_cnt > 1500:
                    #     cap.stop()
                    #     # break
                    #     cap = Video_Buffer(pipe=self.pipe)
                    #     self.frame_ori = cap.get_frame()
