...
from ui.main_new_ui import Ui_MainWindow
...
class LoginWindow(QMainWindow):
    ...
    def check_login_info(self):
        try:
            # print(self.ui_login.id_input.text())
            # print(self.ui_login.pw_input.text())
            ...
            print(f"Error occurred at {current_time}: {e}", file=sys.stderr)
...
#---------------------------------------------------------------------------------------------------------#
    ...
    def shutdown(self):
        ...
        try:
            ...
            print(f"Error occurred while sending exit message at : {e}", file=sys.stderr)
    ...
    def camera_page_display_camera_and_detect_area_list(self, camera_name):
        try:
            ...
            if camera_name in self.camera_info_dict.keys():
                ...
                #                                          host=self.HOST, 
                #                                          port=self.PORT, 
                #                                         #  output_size = (viewer.width(), viewer.height()), 
                #                                          plot_detect=True, 
                #                                          roi_thickness = 2,
    ...
    def init_camera_info(self, reset = False):
        try:
            ...
            if receive_data["success"] == True:
                ...
                self.camera_info_dict = receive_data["data"]

                for camera_name, camera_info in self.camera_info_dict.items():
                    if len(str(camera_info["IP"])) == 0:
                        # num = camera_info["Num"]
                ...
                self.create_fade_out_msg(msg="Invalid ID and PW")

        except Exception as e:
            current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            print(f"Error occurred at {current_time}: {e}", file=sys.stderr)
            self.create_fade_out_msg(msg=f"Error occurred at {current_time}: {e}", file=sys.stderr)
    ...
    try:
        for worker in camera_worker_list:
            ...
            if camera_name == worker.camera_name:
                ...
                event_buffer = worker.event_buffer
                ...
                if len(img_buffer):
                    ...
                    output_event_file_path = os.path.join(ROOT, "backup", nvr_ip, f"{camera_name}", new_date, "events")

                    os.makedirs(output_video_file_path,exist_ok=True)
                    os.makedirs(output_event_file_path,exist_ok=True)

                    output_video_name = os.path.join(output_video_file_path, f"{new_time}_{Eng2kor(alarm[0])}.mp4") 
                    video = cv2.VideoWriter(output_video_name, fourcc, fps, (img_buffer[0].shape[1], img_buffer[0].shape[0]))

                    for img in img_buffer:
                        video.write(img)

                    with open(os.path.join(output_event_file_path, f"{new_time}_{Eng2kor(alarm[0])}.json") , "w", encoding="UTF-8") as f:
                        f.write(json.dumps(event_buffer, indent=4))
