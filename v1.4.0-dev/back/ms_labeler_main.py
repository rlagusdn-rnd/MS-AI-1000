...
from back.ms_labeler_utils import ( 
                                   check_LLM_test,
                                   get_yolo_label, 
                                   get_zero_shot_label, 
                                   save_final_dataset, 
...
TEST = False
VLM_URL = "http://127.0.0.1:1206/vlm_qna_label"
                ...
                with torch.no_grad():
                    ...
                    for video_name in video_name_list:
                        try:
                            ...
                            if zero_shot_flag:
                                # LLM_bbox = check_LLM(model_path = os.getcwd() + "/back/weight/InternVL2-8B-MPO", 
                                #                     img_buffer = img_buffer,
                                #                     label = llm_input_bboxes,
                                #                     verbose = TEST
                                #                     )

                                LLM_bbox = check_LLM_test(img_buffer = img_buffer, label = llm_input_bboxes, VLM_server_url = VLM_URL)
