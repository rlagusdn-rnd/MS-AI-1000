...
def run_LLM(img_buffer : dict, label : dict, VLM_server_url : str):
    for frame_num, img in img_buffer.items():
        ...
        for cls, x1, y1, x2, y2, score in bboxes:
            ...
            cv2.imwrite(os.path.join(VLM_images_dir, f"crop_{timestamp}.jpg"), cropped_img_extend)

            messages = [{
                        "role": "user", 
                        "content": [{"type": "image", "image": os.path.join(VLM_images_dir, f"crop_{timestamp}.jpg")}, 
                                    {"type": "text", "text": "Do you see" + f"{NAMES[cls]}" "in this image and you answer in the following format.\n{anwser: 'yes or no'}"}]
                        }]
            ...
            if response.status_code == 200:
                ...
                print(response)
                try:
                    os.remove(os.path.join(VLM_images_dir, f"crop_{timestamp}.jpg"))
                except:
                    print(os.path.join(VLM_images_dir, f"crop_{timestamp}.jpg") , "파일 삭제 실패")
...
def SAM_label(img_buffer, label, video_name):
    ...
    # with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                ...
                sam2_checkpoint = os.path.join(os.getcwd(),"back", "weights", "segment_anything_2", "sam2.1_hiera_tiny.pt")
