...
class FalldownDetectTester:
    def __init__(self, video_path, yolo_model_path="yolo11n.pt", siglip_model_path="../weights/SigLip_512", save_path="./result"):
        ...
        self.texts = [
                        ...
                        # "a photo of person collapsed on street",

                        "a person collapsed on their hands and knees",
                        "a person crouching on the floor after falling",

                        "a photo of walking person", 
                        "a photo of standing person", 
                        "a photo of sitting person",
    ...
    def run_test(self, skip_frames=30, max_frames=None, conf_threshold=0.33):
        ...
        try:
            while True:
                ...
                processed_count += 1
                if processed_count % 2 ==0 : continue
                ...
                if len(tracks) > 0:
                    for idx, track in enumerate(tracks):
                        ...
                        if len(probs) >= 4:
                            ...
                            if np.max([probs[0], probs[1]]) > 0.80 and all(p < 0.2 for p in probs[4:]):
                            ...
                            elif np.max([probs[0], probs[1]]) > 0.70 and all(p < 0.1 for p in probs[4:]):

                                is_falldown_frame = True
                                falldown_reason = "조건1: falldown 확률 > 50% & 나머지 < 30%"
                            
                            # elif probs[0] > 0.50 and (probs[1] + probs[2]) > 0.15 and all(p < 0.05 for p in probs[3:6]):
                            elif np.max([probs[0], probs[1]]) > 0.2 and (probs[2] + probs[3]) > 0.15 and all(p < 0.05 for p in probs[4:]):
                            ...
                            # 조건 2: argmax가 0이고 나머지 모든 확률이 0.01 미만
                            elif np.max([probs[0], probs[1]]) > 0.1 and all(p < 0.001 for p in probs[4:]):
...
def main_single_video():
    """메인 함수"""
    YOLO_MODEL_PATH = "../weights/yolo/2025-12-02/ms-ai_25-12-02-M.pt"
    SIGLIP_MODEL_PATH = "../weights/SigLip_512"
    ...
    video_path = "./videos/710-4_cam01_swoon01_place01_day_summer.mp4"
    ...
    tester.run_test(skip_frames=30, max_frames=None, conf_threshold=0.33)
