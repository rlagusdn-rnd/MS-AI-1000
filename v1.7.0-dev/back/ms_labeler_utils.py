...
logger = setup_logging(logger_name="MS_LABELER_utils", log_file="MS_LABELER.log")

logger_llm = setup_logging(logger_name="MS_LABELER_LLM", log_file="MS_LABELER_LLM.log")
...
def run_LLM(img_buffer : dict, label : dict, VLM_server_url : str):
    for frame_num, img in img_buffer.items():
        ...
        for cls, x1, y1, x2, y2, score in bboxes:
            ...
            # PIL Image 생성 및 Base64 인코딩 (API 호출을 위해)
            ...
            # Qwen3VL 공식 Base64 형식 사용 (MIME 타입 없음)
            messages = [{
                        "role": "user", 
                        "content": [
                            {"type": "image", "image": f"data:image;base64,{pil_img_base64}"}, 
                            {"type": "text", "text": "You determine " + f"{NAMES[cls]}" + " in this image. " + "Answer in the following format:\n" + "{answer: 'yes or no'}"}
                        ]
                    }]

            # messages = [{"role": "user", "content": [{"type": "image", "image": pil_img}, {"type": "text", "text": "You determine " + f"{NAMES[cls]}" + " in this image. " + "Answer in the following format:\n" + "{{anwser: 'yes or no'}}"}]}],


            response = requests.post(VLM_server_url + "/vlm-QnA",
            ...
            if response.status_code == 200:
                ...
                # print(NAMES[cls], response)
                # logger_llm.info(f"{NAMES[cls]} : {response}")
                # answer 추출 및 파싱
                answer_text = ""
                if isinstance(response["answer"], list) and len(response["answer"]) > 0:
                    answer_text = response["answer"][0]
                elif isinstance(response["answer"], str):
                    answer_text = response["answer"]
                
                # <think> 태그 내용 제거 (있는 경우)
                answer_text = re.sub(r'<think>.*?</think>', '', answer_text, flags=re.DOTALL)
                
                # {answer: 'yes'} 패턴만 정확히 찾기 (대소문자 구분 없이)
                is_yes = False
                answer_match = re.search(r"\{answer:\s*['\"]?(yes|no)['\"]?\}", answer_text, re.IGNORECASE)
                if answer_match:
                    is_yes = (answer_match.group(1).lower() == 'yes')
                # 패턴이 없으면 is_yes는 False로 유지 (데이터 추가 안 됨)
                
                # print(f"답변 파싱 결과: {is_yes} (원본: {answer_text[:100]}...)")
                logger_llm.info(f"{NAMES[cls]} 답변 파싱 결과: {is_yes} (원본: {answer_text[:100]}...)")
...
def get_zero_shot_label(buffer:dict, model_path:str) -> dict:
    """
    Moondream 모델을 사용한 Zero-shot 객체 탐지
    """
    os.environ["HF_MODULES_CACHE"] = model_path
    ...
    for frame_num, img in buffer.items():
        ...
        for text_label in text_labels:
            with torch.no_grad():
                output_text = model.detect(image=pil_img, object=text_label)
...
def nms_sam(boxes, iou_threshold=0.5):
    """
        ...
        if len(cls_boxes) > 1:
            while len(cls_boxes) > 0:
                ...
                for box in cls_boxes:
                    iou_val = calculate_iou(best_box[1:5], box[1:5])
                    # best_box와 box가 완전히 동일한 경우 box는 제거
...
def save_temp_img(img_buffer, img_save_dir):
...
def SAM_label(img_buffer, label):
    # 레이블이 비어있는지 확인
    ...
    # with torch.autocast(device_type='cuda', dtype=torch.bfloat16):
                ...
                save_temp_img(img_buffer = img_buffer, 
                ...
                for frame_num, box_list in label_dict.items():
                    if len(box_list) > 0:
                        final_label[frame_num] = nms_sam(box_list, iou_threshold=0.3)
                    else:
...
def create_dataset_list(dataset_path:str) -> None:
    train_txt = ""
    val_txt = ""
    ...
    for folder_name in folder_list:
        if folder_name not in ["train.txt", "val.txt", "temp"]:
            if folder_name == "auto_labeled_data":
                for nvr_ip in os.listdir(os.path.join(dataset_path, folder_name)):
                    camera_name_path = os.path.join(dataset_path, folder_name, nvr_ip)
                        ...
                        for date in date_list:
                            ...
                            for event in event_list:
                                ...
                                if train_img_list:
                                    train_img_list = sorted(train_img_list)
                                    for img_name in train_img_list:
                                        save_path = os.path.join("./", folder_name, nvr_ip, camera_name, date, event, "images", img_name)
...
def move_dataset_list(dataset_path:str) -> None:
    for nvr_ip in os.listdir(os.path.join(dataset_path, "auto_labeled_data")):
        camera_name_path = os.path.join(dataset_path, "auto_labeled_data", nvr_ip)
        camera_name_list = os.listdir(camera_name_path)
                        ...
                        if train_img_list:
                            ...
                            for img_name in train_img_list:
                                if img_name.split(".")[-1] == "png":
                                    img_path = os.path.join(dataset_path, "auto_labeled_data", nvr_ip, camera_name, date, event, "images", img_name)
                                    try:
                                        shutil.move(img_path, img_data_path)
                                        
                                        label_path = os.path.join(dataset_path, "auto_labeled_data", nvr_ip, camera_name, date, event, "labels", img_name[:-4]+".txt")
...
def remove_empty_folders(directory_path):
    ...
    for nvr_ip in os.listdir(directory_path):
        for camera_name in os.listdir(os.path.join(directory_path, nvr_ip)):
            date_path = os.path.join(directory_path, nvr_ip, camera_name)
