...
def ms_labeler(camera_list_path:str, weight_name:str, zero_shot_flag:str):
    device = "cuda:0" if torch.cuda.is_available() else "cpu"
    yolo_weight_path = os.path.join(os.getcwd(), "back", "weight", "yolo", weight_name)
    zero_shot_flag = True if zero_shot_flag == "1" else False
...
def train(NVR_IP:str, weight_name:str):
    ...
    # os.remove(os.path.join(train_data_path, NVR_IP))
    try:
        shutil.rmtree(os.path.join(train_data_path, NVR_IP))
    except:
        pass
    os.system("chmod 777 -R ./")
